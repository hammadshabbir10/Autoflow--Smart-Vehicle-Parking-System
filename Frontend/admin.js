document.addEventListener("DOMContentLoaded", function () {
    ...
    addSlotBtn.addEventListener("click", async () => {
        ...
        // Validate format: e.g., 10:00 am - 1:00 pm
        const timeFormatRegex = /^([1-9]|1[0-2]).[0-5][0-9]\s?(AM|PM)\s?-\s?([1-9]|1[0-2]).[0-5][0-9]\s?(AM|PM)$/i;
        ...
        if (!timeFormatRegex.test(slotTime)) {
            alert("Invalid format! Please use format like '10.00 AM - 1.00 PM'");
            return;
        }
